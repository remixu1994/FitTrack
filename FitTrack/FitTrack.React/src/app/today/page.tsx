      ...
      <div className="space-y-6">
        <section className="grid gap-6 xl:grid-cols-[minmax(0,1.45fr)_420px]">
          <div className="rounded-[34px] border border-cyan-300/20 bg-[radial-gradient(circle_at_top_left,_rgba(34,211,238,0.18),_transparent_32%),radial-gradient(circle_at_right,_rgba(16,185,129,0.14),_transparent_28%),linear-gradient(180deg,_rgba(8,15,33,0.96),_rgba(2,6,23,0.82))] p-6 shadow-2xl shadow-cyan-950/20">
            <p className="text-xs uppercase tracking-[0.38em] text-cyan-300">{isChinese ? '今日概览' : 'Today Overview'}</p>
            <h3 className="mt-4 max-w-4xl text-3xl font-semibold leading-tight text-white">
              {summary?.headline ?? (isChinese ? '聚焦今天的饮食、训练和恢复。' : 'Focus on food, training, and recovery today.')}
            </h3>
            <p className="mt-4 max-w-3xl text-sm leading-7 text-slate-300">
              {isChinese ? '查看关键指标并快速进入计划与记录页面。' : 'Review key metrics and jump to planning and records.'}
            </p>

            <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              <MetricTile label={isChinese ? '今日热量' : 'Calories Today'} value={`${summary?.caloriesToday?.toFixed(0) ?? '--'} kcal`} />
              <MetricTile label={isChinese ? '今日蛋白' : 'Protein Today'} value={`${summary?.proteinToday?.toFixed(0) ?? '--'} g`} />
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <Link href="/chat" className="rounded-full bg-cyan-300 px-5 py-3 text-xs font-semibold uppercase tracking-[0.3em] text-slate-950">
                {isChinese ? '打开计划中心' : 'Open Planning'}
              </Link>
            </div>
          </div>
        ...
        <section className="grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
          <section className="rounded-[32px] border border-white/10 bg-slate-950/50 p-6">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-xs uppercase tracking-[0.35em] text-emerald-300">{isChinese ? '最近饮食' : 'Recent Meals'}</p>
                <h3 className="mt-3 text-xl font-semibold text-white">{isChinese ? '最近记录的饮食' : 'Latest logged meals'}</h3>
              </div>
            ...
            <div className="mt-5 space-y-3">
              {latestMeals.map((meal) => (
                <div key={meal.id} className="rounded-[24px] border border-white/10 bg-white/5 p-4">
                  ...
                  <p className="mt-3 text-sm text-slate-300">
                    {isChinese ? `蛋白 ${meal.protein} g | 碳水 ${meal.carbs} g | 脂肪 ${meal.fat} g` : `Protein ${meal.protein} g | Carbs ${meal.carbs} g | Fat ${meal.fat} g`}
                  </p>
                </div>
              ))}
              {latestMeals.length === 0 ? (
                <EmptyState
                  text={
                    isChinese
                      ? '今天还没有饮食记录。先记录第一餐，或直接去计划中心查看今天安排。'
                      : 'No food log yet today. Start with your first meal or go to Planning for today’s plan.'
                  }
                />
              ) : null}
            </div>
          ...
          <section className="rounded-[32px] border border-white/10 bg-slate-950/50 p-6">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-xs uppercase tracking-[0.35em] text-violet-300">{isChinese ? '最近训练' : 'Recent Training'}</p>
                <h3 className="mt-3 text-xl font-semibold text-white">{isChinese ? '最近训练记录' : 'Latest workout sessions'}</h3>
              </div>
            ...
            <div className="mt-5 space-y-3">
              ...
              {latestSessions.length === 0 ? (
                <EmptyState
                  text={
                    isChinese
                      ? '今天还没有训练记录。完成一次训练后会显示在这里。'
                      : 'No workout log yet today. Your completed sessions will appear here.'
                  }
                />
              ) : null}
            </div>
