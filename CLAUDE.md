...
├── FitTrack/FitTrack.React/       # Next.js 16 frontend (not in .sln)
└── FitTrack.AgentHost.Python/   # Python agent sidecar (Phase 1)
```

## Primary Delivery Path

`FitTrack.React → FitTrack.Copilot API` (NOT Blazor)

- **FitTrack**: Legacy/reference, no new features
- **FitTrack.Copilot**: Main backend — API, AI agents, database writes
- **FitTrack.React**: Main frontend — chat, food records, workouts, progress pages

## Key Rules

- Frontend changes → `FitTrack.React`, NOT `FitTrack.Copilot/Components/*`
...
| FitTrack.React | `cd FitTrack/FitTrack.React && npm install && npm run dev` | http://localhost:3000 |

## Architecture

```
FitTrack.React → FitTrack.Copilot → SQLite / USDA API / Azure OpenAI
                              ↓
...
- `FitTrack/FitTrack.React/src/components/chat/chat-view.tsx` — main chat workbench
- `FitTrack/FitTrack.React/src/lib/http.ts` — auth, refresh, 401 retry
